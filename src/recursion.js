...
var arraySum = function(array) {
    if (array.length === 0) {
        ...
        var flattened = array.flat(Infinity);
        var [first, ...rest] = flattened;
...
countKeysInObj = function(obj, key) {
    ...
    for (var keys in obj) {
        ...
        var currentValue = obj[keys];
        if (typeof currentValue === 'object') {
            count += countKeysInObj(currentValue, key);
        }
    }
    return count;
};
...
var capitalizeWords = function(array) {
    ...
    return [currentWord.toUpperCase()].concat(capitalizeWords(array.slice(1)));
};
...
var capitalizeFirst = function(array) {
    ...
    var result = currentWord.replace(currentLetter, currentLetter.toUpperCase());
...
var flatten = function(array) {
    ...
    for (var i = 0; i < array.length; i++) {
        ...
        if (Array.isArray(currentItem)) {
            result = result.concat(flatten(currentItem));
        } else {
...
var alternateSign = function(array) {
    ...
    var mustBePositive = array[0];
    if (mustBePositive < 0) {
        mustBePositive = ~mustBePositive + 1;
    }
    if (array.length === 1) {
        return [mustBePositive];
    }
    var mustBeNegative = array[1];
    if (mustBeNegative > 0) {
        mustBeNegative = ~mustBeNegative + 1;
    }
    return [mustBePositive, mustBeNegative].concat(alternateSign(array.slice(2)));
};
